...
async function fetchPage(col, startDate) {
    ...
    let q = query(
        ...
        orderBy(col === "finances" ? "createdAt" : "soldAt"),
        limit(PAGE_SIZE)
    );
...
function updateAnalysis(stats) {
    ...
    if (stats.ventes > stats.entree) {
        message += " Les ventes représentent une source importante de revenus.";
    }
...
function checkAlerts(stats) {
    ...
    if (stats.entree < stats.sortie) alerts.push("Perte détectée");
    if (stats.stock < 10) alerts.push("Stock critique");
    if (stats.ventes === 0) alerts.push("Aucune vente sur la période");
